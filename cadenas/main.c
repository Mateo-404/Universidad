...
int PuntajeGanado(int numPregunta, int puntaje, int checkpont){
    ...
    // Del 5 al 11 || 13 al 15, comparten la misma fórmula
...
int main(){
    int menu, i, j, numpreguntas[15], puntaje, checkpoint, repetido, auxx, w, flag, respuesta, rtaa[30], contador_preguntas;
    char aux[20];

    puntaje = checkpoint = menu = contador_preguntas = 0;
    srand(time(0));
    ...
    do
    {
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    for (i = 0; i < 15; i++) {
                        ...
                        numpreguntas[i] = rand() % contador_preguntas;
                    ...
                    for (w=0; w < 15; w++){
                        ...
                        if (respuesta == rtaa[numpreguntas[w]]){
                            ...
                            puntaje = PuntajeGanado(w, puntaje, checkpoint);
                            //Si el usuario desea continuar o no
                            if (Eleccion(puntaje, ranking[0].puntaje) == 1){
        ...
        if (menu == 3){
            if (flag != 1){
                ...
                printf("\n Tu nombre de usuario es %s y tu puntaje es de %d USD \n\n Recuerda que para cambiar de usuario es necesario reiniciar el juego. \n\n (Pulse ENTER para continuar) \n\n", ranking[10].nombre, puntaje);
                getchar(); system("cls");
            }
        }
        if (menu == 4){
